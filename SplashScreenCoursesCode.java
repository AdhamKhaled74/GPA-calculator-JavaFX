...
public class SplashScreenCoursesCode extends Application {
    private  Stage primaryStage;
    private static final int SPLASH_DURATION = 1500; // Duration in milliseconds
    ...
    private void simulateLoading(Label progressLabel, Stage primaryStage) {
        Thread loadingThread = new Thread(() -> {
            ...
            while (completedTasks < TOTAL_TASKS) {
                ...
                updateLabel(progressLabel, "Loading student Details.." + String.format("%.0f%%", progress));
    ...
    private void closeSplashScreen(Window window) {
        ...
        Platform.runLater(() -> {
            window.hide();
            CourseTableFromDatabase courseTableFromDatabase = new CourseTableFromDatabase(); // Replace with your LoginScreen class
            Stage loginStage = new Stage();
            courseTableFromDatabase.start(loginStage);
